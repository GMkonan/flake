...
in {
  ...
  programs.tmux = {
    ...
    extraConfig =
      #tmux
      ''
        # Preserve modified Enter keys for Pi under Ghostty + tmux.
        ...
        # Use Pi's semantic task title only while Pi is the active process.
        # Other windows keep their short tmux name instead of Ghostty's verbose pane title.
        setw -g allow-set-title on
        set -g set-titles on
        set -g set-titles-string "#S · #I:#{?#{==:#{pane_current_command},pi},#T,#W}"
        ...
        set -g window-status-format " #I:#{?#{==:#{pane_current_command},pi},#T,#W} "
        set -g window-status-current-format " #I:#{?#{==:#{pane_current_command},pi},#T,#W} "
        ...
        set -g status-right "#[fg=${theme.colors.textMuted},bg=${theme.colors.surface}] #(awk '{ d=int($1/86400); h=int(($1%86400)/3600); printf \"up %dd %dh\", d, h }' /proc/uptime) "
      '';
  };
}
